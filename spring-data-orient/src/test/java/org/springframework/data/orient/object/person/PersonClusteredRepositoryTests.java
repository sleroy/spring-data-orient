...
public class PersonClusteredRepositoryTests extends AbstractTestNGSpringContextTests {
	...
	public void before() {
		...
		this.operations
		.command("insert into cluster:person (firstName, lastName, active) values ('Dzmitry', 'Naskou', true)");
		this.operations
		.command("insert into cluster:person (firstName, lastName, active) values ('Koby', 'Eliot', true)");
		this.operations
		.command("insert into cluster:person_tmp (firstName, lastName, active) values ('Ronny', 'Carlisle', true)");
		this.operations
		.command("insert into cluster:person_tmp (firstName, lastName, active) values ('Jameson', 'Matthew', true)");
		this.operations
		.command("insert into cluster:person_history (firstName, lastName, active) values ('Roydon', 'Brenden', false)");
	}
	...
	public void countPersonTmp() {
		Assert.assertEquals(this.tmpRepository.count(), 2);
	}
	...
	public void savePersonToTmpCluster() {
		...
		final String rid = this.tmpRepository.save(person).getRid();

		final Person result = this.tmpRepository.findOne(rid);
	...
	public void tmpRepositoryAutowiring() {
		Assert.assertNotNull(this.tmpRepository);
	}
}
